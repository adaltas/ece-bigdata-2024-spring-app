...
If still having trouble, contact your teacher via email using the tag `[<School name>][VPN]` and attach:
...
2. Check the email you received from Adaltas. There you will find your username, password, name of the group you belong to, and a sample SSH command to connect to the cluster. If my username is `john`, this is the command I received:
   ```
   ssh john@edge-1.au.adaltas.cloud
   ```
3. On your first connection, you will be asked to change your password. Read the instructions carefully: when you are prompted `Current password`, type the temporary password that you received by email. Only after you will need to type and re-type your new password. Please store your new password in a safe place, since you will need it each time to connect.
...
Ultimately, you have to create a file in the folder `/home/$USER/lab1`. The file has to:

- Be named after your username with the extension `.txt` (e.g. `john.txt`)
- Contain some random text.
